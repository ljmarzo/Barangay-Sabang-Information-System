...
Public Class BrgyClearance
    ...
    Private Sub LoadPurpose()
        Try
            Dim con As New SqlConnection
            Dim cmd As New SqlCommand("SELECT Purpose FROM Purpose", con)
            con.ConnectionString = "Data Source = LJMARZO-PC; Initial Catalog = BrgySabangInfo; Integrated Security = True"
            Dim ad As New SqlDataAdapter(cmd)
            Dim tbl As New DataTable()

            ad.Fill(tbl)
            PurposeComboBox.DataSource = tbl
            PurposeComboBox.DisplayMember = "Purpose"
            PurposeComboBox.ValueMember = "Purpose"
        Catch
            MessageBox.Show("Error on Retrieving Data", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    ...
    Public Sub BrgyClearance_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        GetLogForToolStrip()
        SearchTextBox.Focus()
        ...
        Try
            Dim con As New SqlConnection("Data Source =  LJMARZO-PC; Initial Catalog = BrgySabangInfo; Integrated Security = True")
            Dim cmd As SqlCommand
            cmd = New SqlCommand("Select Count(*) from Logs where Action ='Printed a Brgy Clearance' and year(date) =  YEAR(getdate()) ", con)
            con.Open()
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles PreviewButton.Click
        ...
        Try
            If PurposeComboBox.Text = "" Then
                MessageBox.Show("Please enter purpose!", "Error Preview", MessageBoxButtons.OK, MessageBoxIcon.Error)
            ElseIf logTextBox.Text <> Date.Now.ToString("yy") & "-" & no1 Then
    ...
    Private Sub PrintButton_Click(sender As Object, e As EventArgs) Handles PrintButton.Click
        Try
            ...
            If PurposeComboBox.Text = "" And logTextBox.Text <> Date.Now.ToString("yy") & "-" & no1 Then
                MessageBox.Show("Error!", "Error Preview", MessageBoxButtons.OK, MessageBoxIcon.Error)
            ...
            Else
                ...
                If (dr = DialogResult.OK) Then
                    ...
                    End If
                    Dim action = "Printed a Brgy Clearance"
                    ...
                    SearchComboBox.Text = "Surname"
                    ShowPeopleList()
                    PurposeComboBox.Text = "SCHOLARSHIP"
                    LoadPurpose()
                    Try
                        Dim con As New SqlConnection("Data Source =  LJMARZO-PC; Initial Catalog = BrgySabangInfo; Integrated Security = True")
                        Dim cmd As SqlCommand
